...
- **D-B4 — G4 scindé** : B-cœur (schéma + persistance + lecture + fusion G2 spatiale) et B-app (markers/centroïdes) d'abord ; la réécriture du workflow de validation terrain QField (`validation_*` sur clé pixel) **livrée en Phase B.2** (`nemeton` v0.93.1, 2026-06-19) : `ingest_health_validation()` snappe sur `alert.geom_wkt` filtré par `alert.zone_id`, plus de `JOIN plot`. Découplage de la placette du suivi sanitaire **complet** côté cœur.
